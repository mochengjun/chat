...
class _ChatRoomViewState extends State<_ChatRoomView> {
  ...
  void initState() {
    super.initState();
    _loadCurrentUser();
    // 通知全局 RoomListBloc 标记此房间已读，清零未读数
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<RoomListBloc>().add(MarkRoomAsRead(widget.roomId));
    });
  }
  ...
  Future<void> _loadCurrentUser() async {
    ...
    if (userId != null && userId.isNotEmpty) {
      ...
      // 如果没有保存用户 ID，使用占位符（不应该发生，因为登录时已保存）
      print('Warning: No user ID found in secure storage');
      setState(() => _currentUserId = '');
    }
  }
  ...
  Widget _buildMessageList(BuildContext context, ChatRoomState state) {
    ...
    if (state is ChatRoomLoaded) {
      ...
      // 构建消息列表
      final messageListView = NotificationListener<ScrollNotification>(
        ...
        child: ListView.builder(
          ...
          findChildIndexCallback: (Key key) {
            if (key is ValueKey<String>) {
              ...
              return state.messages.length - 1 - idx;
            }
            return null;
          },
          ...
          itemBuilder: (context, index) {
            ...
            if (reversedIndex > 0) {
              ...
              showDateSeparator = prevDate.isBefore(currentDate);
            }
                ...
                if (showDateSeparator)
                  Padding(
                    ...
                    child: Container(
                      ...
                      child: Text(
                        TimeUtils.getRelativeDateDescription(message.createdAt),
      ...
      // 直接返回消息列表，不再使用 SelectionArea 包裹
      // 因为 SelectionArea 与 SelectableText 会冲突，导致右键菜单只显示 "Select All"
      // 文本选择和复制功能由 message_bubble.dart 中的 SelectableText 处理
