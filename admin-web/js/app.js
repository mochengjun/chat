...
function renderSettings(settings) {
    ...
    const isSuperAdmin = AppState.adminInfo?.role === 'super_admin';
    
    if (settings.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:#999;">暂无系统设置</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = settings.map(setting => `
        <tr>
            ...
            <td><span class="setting-value-cell">${escapeHtml(setting.value)}</span></td>
            <td class="truncate" title="${escapeHtml(setting.description || '')}">${escapeHtml(setting.description || '-')}</td>
            <td>${formatDateTime(setting.updated_at)}</td>
            <td>${escapeHtml(setting.updated_by || '-')}</td>
            <td>
                ${isSuperAdmin ? `
                    <button class="btn btn-small btn-secondary" onclick="showEditSettingModal('${escapeHtml(setting.key)}')">
                        <i class="fas fa-edit"></i> 编辑
                    </button>
                    <button class="btn btn-small btn-danger" onclick="confirmDeleteSetting('${escapeHtml(setting.key)}')">
...
function showEditSettingModal(key) {
    elements.modalTitle.textContent = '编辑设置';
    elements.modalBody.innerHTML = `
        <div class="form-group">
            <label>设置键名</label>
            <input type="text" id="edit-setting-key" disabled>
        </div>
        <div class="form-group">
            <label>设置值</label>
            <input type="text" id="edit-setting-value" placeholder="加载中...">
        </div>
        <div class="form-group">
            <label>描述</label>
            <input type="text" id="edit-setting-description" placeholder="加载中...">
        </div>
    `;
    elements.modalFooter.innerHTML = `
        <button class="btn btn-secondary" onclick="closeModal()">取消</button>
        <button class="btn btn-primary" id="edit-setting-save-btn" disabled>保存</button>
    `;
    showModal();

    // 安全赋值：通过 DOM 属性设置，避免 XSS
    document.getElementById('edit-setting-key').value = key;

    // 从 API 获取最新值
    AdminAPI.getSetting(key).then(setting => {
        ...
        saveBtn.onclick = () => updateSetting(key);
    }).catch(err => {
        showToast('获取设置详情失败: ' + err.message, 'error');
        closeModal();
    });
}
...
function confirmDeleteSetting(key) {
    elements.modalTitle.textContent = '删除设置';
    elements.modalBody.innerHTML = `
        <div style="text-align:center;padding:20px 0;">
            <i class="fas fa-exclamation-triangle" style="font-size:48px;color:var(--danger-color);margin-bottom:16px;display:block;"></i>
            <p>确定要删除设置 <strong>${escapeHtml(key)}</strong> 吗？</p>
            <p style="color:#999;font-size:13px;">此操作不可撤销，设置项将被永久删除。</p>
        </div>
    `;
    elements.modalFooter.innerHTML = `
        <button class="btn btn-secondary" onclick="closeModal()">取消</button>
        <button class="btn btn-danger" onclick="deleteSetting('${escapeHtml(key)}')">确认删除</button>
    `;
    showModal();
}
