...
export function ChatRoomListPage() {
  ...
  const { rooms, currentRoom, isLoadingRooms, fetchRooms, createRoom } = useChatStore();
  ...
  // 初始化：获取房间列表（全局监听器已在MainLayout中统一初始化）
  useEffect(() => {
    fetchRooms();
  }, [fetchRooms]);
  ...
  const handleRoomClick = (room: Room) => {
    // 仅导航，不调用 setCurrentRoom（由 ChatRoomPage 的 useEffect 负责初始化）
    // 避免双重 fetchMessages/fetchReadReceipts/markAsRead
