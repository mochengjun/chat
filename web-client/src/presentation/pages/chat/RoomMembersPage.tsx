...
import type { MemberResponse, MembersListResponse, UserSearchResponse, UserSearchResult } from '@shared/types/api.types';
...
function mapMember(data: MemberResponse, roomId: string): Member {
  return {
    ...
    user: {
      id: data.user_id,
      // MemberResponse 不含 username 字段，使用 display_name 作为回退
      username: data.display_name || data.user_id,
      email: '',
...
export function RoomMembersPage() {
  ...
  const handleSearch = async (value: string) => {
    ...
    try {
      ...
      setSearchResults((response.data.users || []).filter(u => !existingMemberIds.has(u.user_id)));
    } catch {
