...
// 房间初始化保护：记录房间初始化时间
const roomInitTimestamps = new Map<string, number>();
const INIT_PROTECTION_DURATION = 500; // 500ms 初始化保护期
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  handleNewMessage: (messageData: MessageResponse) => {
    ...
    if (processedMessageIds.has(message.id)) {
      console.log(`[ChatStore] 跳过重复消息: ${message.id}`);
      return;
    }
    processedMessageIds.add(message.id);
    
    // 初始化保护期检查：如果房间刚初始化，跳过消息处理
    const initTime = roomInitTimestamps.get(message.roomId);
    if (initTime && Date.now() - initTime < INIT_PROTECTION_DURATION) {
      console.log(`[ChatStore] 跳过初始化期间的消息: ${message.id}`);
      return;
    }
