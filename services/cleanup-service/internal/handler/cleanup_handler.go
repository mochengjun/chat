...
func (h *CleanupHandler) PerformCleanup() error {
	...
	log.Printf("  - Auto-deleted messages: %d", result.AutoDeletedMessages)
...
// PerformAutoDeleteCleanup 执行自动删除消息清理（定时撤回）
func (h *CleanupHandler) PerformAutoDeleteCleanup() (int64, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 1*time.Minute)
	defer cancel()

	return h.service.PerformAutoDeleteCleanup(ctx)
}
