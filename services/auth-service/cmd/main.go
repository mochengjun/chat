...
func main() {
	...
	if oauthConfig.IsConfigured() {
		oauthService = service.NewOAuthService(oauthConfig, userRepo, deviceRepo, tokenRepo, oauthRepo, jwtSecret)
	...
	if oauthService != nil {
		oauthHandler = handler.NewOAuthHandler(oauthService, oauthConfig)
	}
	...
	v1 := router.Group("/api/v1")
	{
		...
		protected.Use(authMiddleware.Authenticate())
		{
			...
			// OAuth 账户管理（需要认证）
			if oauthHandler != nil {
				protected.GET("/auth/oauth/accounts", oauthHandler.GetOAuthAccounts)
				protected.DELETE("/auth/oauth/accounts/:provider", oauthHandler.UnlinkOAuthAccount)
				protected.GET("/auth/oauth/google/link", oauthHandler.GoogleLink)
			}
			...
			chat := protected.Group("/chat")
			{
				...
				rooms := chat.Group("/rooms")
				{
					...
					rooms.GET("/public", chatHandler.ListPublicRooms)
			...
			admin := protected.Group("/admin")
			{
				...
				settings := admin.Group("/settings")
				{
					...
					settings.DELETE("/:key", adminHandler.DeleteSetting)
				}
	...
	// 内部 API（供 cleanup-service 等内部服务调用）
	internal := router.Group("/internal")
	internal.Use(internalAuthMiddleware())
	{
		internal.POST("/messages/auto-delete", chatHandler.AutoDeleteMessages)
	}
...
func autoMigrate(db *gorm.DB) error {
	return db.AutoMigrate(
		...
		&repository.OAuthAccount{}, // OAuth账户表
	)
}
