...
func (s *pushService) RegisterToken(ctx context.Context, userID, deviceID string, platform repository.PushPlatform, token string) error {
	...
	return s.pushRepo.SaveToken(ctx, pushToken)
}

func (s *pushService) UnregisterToken(ctx context.Context, token string) error {
	return s.pushRepo.DeactivateToken(ctx, token)
}
...
func (s *pushService) SendToUser(ctx context.Context, payload *PushPayload) ([]PushResult, error) {
	// 检查用户推送设置
	settings, err := s.pushRepo.GetUserSettings(ctx, payload.UserID)
	if err != nil {
		return nil, err
	}
	...
	// 获取用户所有活跃的推送 token
	tokens, err := s.pushRepo.GetActiveTokensByUser(ctx, payload.UserID)
	if err != nil {
		return nil, err
	}
	...
	for _, token := range tokens {
		...
		// 记录推送
		s.recordNotification(ctx, payload, token, result)
	}
...
func (s *pushService) SendToDevice(ctx context.Context, userID, deviceID string, payload *PushPayload) (*PushResult, error) {
	token, err := s.pushRepo.GetTokenByDevice(ctx, userID, deviceID)
	if err != nil {
		return nil, err
	}
	if token == nil {
	...
	settings, _ := s.pushRepo.GetUserSettings(ctx, userID)
	if settings == nil {
...
func (s *pushService) ProcessPendingNotifications(ctx context.Context) error {
	...
	for _, notification := range notifications {
		// 获取用户的推送 tokens
		tokens, err := s.pushRepo.GetActiveTokensByUser(ctx, notification.UserID)
		if err != nil || len(tokens) == 0 {
			s.pushRepo.UpdateNotificationStatus(ctx, notification.ID, repository.PushStatusFailed, "", "No active tokens")
			continue
		}
		...
		for _, token := range tokens {
			...
			if result.Success {
				s.pushRepo.UpdateNotificationStatus(ctx, notification.ID, repository.PushStatusSent, result.MessageID, "")
				break
			} else {
				s.pushRepo.UpdateNotificationStatus(ctx, notification.ID, repository.PushStatusFailed, "", result.ErrorMessage)
			}
		}
	}
	...
	case repository.PushPlatformFCM:
		...
		if err != nil {
			...
			if isInvalidTokenError(err) {
				s.pushRepo.DeactivateToken(ctx, token.Token)
			}
		} else {
			result.Success = true
...
func (s *pushService) sendRawNotification(ctx context.Context, token *repository.PushToken, notification *repository.PushNotification) PushResult {
	...
	settings, _ := s.pushRepo.GetUserSettings(ctx, notification.UserID)
	if settings == nil {
		settings = &repository.UserPushSettings{EnablePush: true, EnableSound: true}
	}
...
func (s *pushService) recordNotification(ctx context.Context, payload *PushPayload, token *repository.PushToken, result PushResult) {
	...
	s.pushRepo.CreateNotification(ctx, notification)
}
