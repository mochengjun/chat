...
type AdminService interface {
	...
	DeleteSetting(ctx context.Context, adminUserID, key string, ipAddress, userAgent string) error
...
func (s *adminService) CheckAdminAccess(ctx context.Context, userID string, requiredRoles ...repository.AdminRole) error {
	isAdmin, role, err := s.adminRepo.IsAdmin(ctx, userID)
	if err != nil {
...
func (s *adminService) GetUsers(ctx context.Context, adminUserID string, page, pageSize int, search string, activeOnly bool) (*UserListResponse, error) {
	...
	users, total, err := s.adminRepo.GetUsers(ctx, page, pageSize, search, activeOnly)
	if err != nil {
...
func (s *adminService) UpdateUserStatus(ctx context.Context, adminUserID, targetUserID string, isActive bool, ipAddress, userAgent string) error {
	...
	// 获取目标用户信息（用于审计日志）
	targetUser, err := s.adminRepo.GetUserByID(ctx, targetUserID)
	if err != nil {
		return fmt.Errorf("user not found: %w", err)
	}

	if err := s.adminRepo.UpdateUserStatus(ctx, targetUserID, isActive); err != nil {
		return fmt.Errorf("failed to update user status: %w", err)
	}

	// 记录审计日志
	adminUser, _ := s.adminRepo.GetUserByID(ctx, adminUserID)
	...
	s.createAuditLog(ctx, repository.AuditActionUserUpdate, adminUserID, actorName, "user", targetUserID, targetUser.Username, fmt.Sprintf("User %s", action), ipAddress, userAgent)
...
func (s *adminService) ResetUserPassword(ctx context.Context, adminUserID, targetUserID, newPassword string, ipAddress, userAgent string) error {
	...
	if err := s.adminRepo.ResetUserPassword(ctx, targetUserID, string(hashedPassword)); err != nil {
	...
	s.createAuditLog(ctx, repository.AuditActionUserUpdate, adminUserID, actorName, "user", targetUserID, targetUser.Username, "Password reset by admin", ipAddress, userAgent)
...
func (s *adminService) DeleteUser(ctx context.Context, adminUserID, targetUserID string, ipAddress, userAgent string) error {
	...
	if err := s.adminRepo.DeleteUser(ctx, targetUserID); err != nil {
	...
	s.createAuditLog(ctx, repository.AuditActionUserDelete, adminUserID, actorName, "user", targetUserID, targetUser.Username, "User deleted", ipAddress, userAgent)
...
func (s *adminService) GetAdminUsers(ctx context.Context, adminUserID string) ([]*AdminUserInfo, error) {
	...
	admins, err := s.adminRepo.GetAdminUsers(ctx)
	if err != nil {
	...
	for _, admin := range admins {
		user, err := s.adminRepo.GetUserByID(ctx, admin.UserID)
		if err != nil {
			continue // 跳过找不到用户的记录
		}
...
func (s *adminService) CreateAdminUser(ctx context.Context, adminUserID string, req *CreateAdminRequest, ipAddress, userAgent string) error {
	...
	// 验证目标用户存在
	targetUser, err := s.adminRepo.GetUserByID(ctx, req.UserID)
	if err != nil {
		return fmt.Errorf("user not found: %w", err)
	}

	// 检查是否已是管理员
	isAdmin, _, _ := s.adminRepo.IsAdmin(ctx, req.UserID)
	if isAdmin {
	...
	s.createAuditLog(ctx, repository.AuditActionAdminAction, adminUserID, actorName, "admin", req.UserID, targetUser.Username, fmt.Sprintf("Granted %s role", req.Role), ipAddress, userAgent)
...
func (s *adminService) UpdateAdminRole(ctx context.Context, adminUserID, targetUserID string, role repository.AdminRole, ipAddress, userAgent string) error {
	...
	// 验证目标用户是管理员
	targetAdmin, err := s.adminRepo.GetAdminUser(ctx, targetUserID)
	if err != nil {
	...
	if targetAdmin.Role == repository.AdminRoleSuperAdmin && role != repository.AdminRoleSuperAdmin {
		admins, _ := s.adminRepo.GetAdminUsers(ctx)
		superAdminCount := 0
	...
	if err := s.adminRepo.UpdateAdminRole(ctx, targetUserID, role); err != nil {
	...
	targetUser, _ := s.adminRepo.GetUserByID(ctx, targetUserID)
	targetName := targetUserID
	if targetUser != nil {
		targetName = targetUser.Username
	}

	s.createAuditLog(ctx, repository.AuditActionAdminAction, adminUserID, actorName, "admin", targetUserID, targetName, fmt.Sprintf("Role changed from %s to %s", oldRole, role), ipAddress, userAgent)
...
func (s *adminService) DeleteAdminUser(ctx context.Context, adminUserID, targetUserID string, ipAddress, userAgent string) error {
	...
	if err := s.adminRepo.DeleteAdminUser(ctx, targetUserID); err != nil {
	...
	s.createAuditLog(ctx, repository.AuditActionAdminAction, adminUserID, actorName, "admin", targetUserID, targetName, fmt.Sprintf("Removed %s role", targetAdmin.Role), ipAddress, userAgent)
...
func (s *adminService) GetRooms(ctx context.Context, adminUserID string, page, pageSize int, search, roomType string) (*RoomListResponse, error) {
	...
	rooms, total, err := s.adminRepo.GetRooms(ctx, page, pageSize, search, roomType)
	if err != nil {
...
func (s *adminService) GetRoom(ctx context.Context, adminUserID, roomID string) (*repository.Room, error) {
	...
	room, err := s.adminRepo.GetRoomByID(ctx, roomID)
	if err != nil {
...
func (s *adminService) GetRoomMembers(ctx context.Context, adminUserID, roomID string) ([]*repository.RoomMember, error) {
	...
	members, err := s.adminRepo.GetRoomMembersAdmin(ctx, roomID)
	if err != nil {
...
func (s *adminService) DeleteRoom(ctx context.Context, adminUserID, roomID string, ipAddress, userAgent string) error {
	...
	if err := s.adminRepo.DeleteRoom(ctx, roomID); err != nil {
	...
	s.createAuditLog(ctx, repository.AuditActionRoomDelete, adminUserID, actorName, "room", roomID, roomName, fmt.Sprintf("Room deleted (type: %s)", room.Type), ipAddress, userAgent)
...
func (s *adminService) GetAuditLogs(ctx context.Context, adminUserID string, page, pageSize int, action, actorID string, startTime, endTime *time.Time) (*AuditLogListResponse, error) {
	...
	logs, total, err := s.adminRepo.GetAuditLogs(ctx, page, pageSize, action, actorID, startTime, endTime)
	if err != nil {
...
func (s *adminService) UpdateSetting(ctx context.Context, adminUserID, key string, req *UpdateSettingRequest, ipAddress, userAgent string) error {
	...
	if err := s.adminRepo.UpdateSetting(ctx, setting); err != nil {
	...
	s.createAuditLog(ctx, repository.AuditActionSettingUpdate, adminUserID, actorName, "setting", key, key, details, ipAddress, userAgent)

	return nil
}

func (s *adminService) DeleteSetting(ctx context.Context, adminUserID, key string, ipAddress, userAgent string) error {
	if err := s.CheckAdminAccess(ctx, adminUserID, repository.AdminRoleSuperAdmin); err != nil {
		return err
	}
	...
	if err := s.adminRepo.DeleteSetting(ctx, key); err != nil {
	...
	details := fmt.Sprintf("Setting '%s' deleted (value was: '%s')", key, setting.Value)
	s.createAuditLog(ctx, repository.AuditActionSettingDelete, adminUserID, actorName, "setting", key, key, details, ipAddress, userAgent)
...
func (s *adminService) GetStats(ctx context.Context, adminUserID string) (*repository.SystemStats, error) {
	...
	roomStats, err := s.adminRepo.GetRoomStats(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to get room stats: %w", err)
	}

	messageStats, err := s.adminRepo.GetMessageStats(ctx)
	if err != nil {
...
func (s *adminService) GetRoomStats(ctx context.Context, adminUserID string) (*repository.RoomStats, error) {
	...
	return s.adminRepo.GetRoomStats(ctx)
}
...
func (s *adminService) GetMessageStats(ctx context.Context, adminUserID string) (*repository.MessageStats, error) {
	...
	return s.adminRepo.GetMessageStats(ctx)
}

// ====== 辅助方法 ======

func (s *adminService) createAuditLog(ctx context.Context, action repository.AuditAction, actorID, actorName, targetType, targetID, targetName, details, ipAddress, userAgent string) {
	...
	// 忽略错误，审计日志失败不应影响主操作
	_ = s.adminRepo.CreateAuditLog(ctx, log)
}
