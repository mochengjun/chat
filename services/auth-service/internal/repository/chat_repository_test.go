...
func (s *ChatRepositoryTestSuite) TestCreateRoom() {
	...
	err := s.repo.CreateRoom(s.ctx, room)
	s.NoError(err)

	found, err := s.repo.GetRoom(s.ctx, "room-001")
	s.NoError(err)
...
func (s *ChatRepositoryTestSuite) TestCreateDirectRoom() {
	...
	// 添加两个成员
	s.repo.AddMember(s.ctx, &repository.RoomMember{
		ID:     "member-001",
		RoomID: "dm-001",
		UserID: "user-001",
		Role:   "member",
	...
	// 验证成员存在
	isMember, err := s.repo.IsMember(s.ctx, "dm-001", "user-001")
	s.NoError(err)
	s.True(isMember)

	isMember2, err := s.repo.IsMember(s.ctx, "dm-001", "user-002")
	s.NoError(err)
	s.True(isMember2)
}
...
func (s *ChatRepositoryTestSuite) TestUpdateRoom() {
	...
	s.repo.CreateRoom(s.ctx, room)

	// 更新房间
	room.Name = "Updated Name"
	room.Description = "New description"
	err := s.repo.UpdateRoom(s.ctx, room)
	s.NoError(err)

	found, _ := s.repo.GetRoom(s.ctx, "room-update")
...
func (s *ChatRepositoryTestSuite) TestDeleteRoom() {
	...
	err := s.repo.DeleteRoom(s.ctx, "room-delete")
	s.NoError(err)

	found, err := s.repo.GetRoom(s.ctx, "room-delete")
	s.NoError(err)
	s.Nil(found)
}
...
func (s *ChatRepositoryTestSuite) TestGetUserRooms() {
	...
	rooms, err := s.repo.GetUserRooms(s.ctx, "user-001")
	s.NoError(err)
	s.Len(rooms, 5)
}
...
func (s *ChatRepositoryTestSuite) TestAddMember() {
	...
	err := s.repo.AddMember(s.ctx, member)
	s.NoError(err)

	// 验证成员已添加
	isMember, err := s.repo.IsMember(s.ctx, "member-room", "user-001")
	s.NoError(err)
	s.True(isMember)
}
...
func (s *ChatRepositoryTestSuite) TestRemoveMember() {
	...
	// 移除成员
	err := s.repo.RemoveMember(s.ctx, "remove-member-room", "user-002")
	s.NoError(err)

	isMember, _ := s.repo.IsMember(s.ctx, "remove-member-room", "user-002")
	s.False(isMember)
}
...
func (s *ChatRepositoryTestSuite) TestGetRoomMembers() {
	...
	// 添加多个成员
	s.repo.AddMember(s.ctx, &repository.RoomMember{ID: "lm-1", RoomID: "list-members-room", UserID: "user-001", Role: "admin"})
	s.repo.AddMember(s.ctx, &repository.RoomMember{ID: "lm-2", RoomID: "list-members-room", UserID: "user-002", Role: "member"})
	s.repo.AddMember(s.ctx, &repository.RoomMember{ID: "lm-3", RoomID: "list-members-room", UserID: "user-003", Role: "member"})

	members, err := s.repo.GetRoomMembers(s.ctx, "list-members-room")
	s.NoError(err)
	s.Len(members, 3)
}
...
func (s *ChatRepositoryTestSuite) TestUpdateMember() {
	...
	s.repo.AddMember(s.ctx, member)

	// 升级为管理员
	member.Role = "admin"
	err := s.repo.UpdateMember(s.ctx, member)
	s.NoError(err)

	found, _ := s.repo.GetMember(s.ctx, "role-room", "user-002")
...
func (s *ChatRepositoryTestSuite) TestCreateMessage() {
	...
	err := s.repo.CreateMessage(s.ctx, msg)
	s.NoError(err)

	found, err := s.repo.GetMessage(s.ctx, "msg-001")
	s.NoError(err)
...
func (s *ChatRepositoryTestSuite) TestGetMessages() {
	...
	for i := 1; i <= 25; i++ {
		...
		s.repo.CreateMessage(s.ctx, msg)
		time.Sleep(time.Millisecond) // 确保时间戳不同
	}

	// 获取消息（分页）
	messages, err := s.repo.GetMessages(s.ctx, "paginate-msg-room", 20, "")
	s.NoError(err)
...
func (s *ChatRepositoryTestSuite) TestDeleteMessage() {
	...
	err := s.repo.DeleteMessage(s.ctx, "delete-msg")
	s.NoError(err)

	// 软删除 - 消息仍存在但标记为已删除
	found, err := s.repo.GetMessage(s.ctx, "delete-msg")
	s.NoError(err)
	s.NotNil(found)
...
func (s *ChatRepositoryTestSuite) TestUpdateMessage() {
	...
	err := s.repo.UpdateMessage(s.ctx, msg)
	s.NoError(err)

	found, _ := s.repo.GetMessage(s.ctx, "status-msg")
...
func (s *ChatRepositoryTestSuite) TestReadReceipt() {
	...
	err := s.repo.UpdateReadReceipt(s.ctx, receipt)
	s.NoError(err)

	// 获取已读回执
	found, err := s.repo.GetReadReceipt(s.ctx, "read-room", "user-002")
	s.NoError(err)
...
func (s *ChatRepositoryTestSuite) TestGetUnreadCount() {
	...
	// 添加成员
	s.repo.AddMember(s.ctx, &repository.RoomMember{ID: "unread-mem", RoomID: "unread-room", UserID: "user-002", Role: "member"})
	...
	// 获取未读数量
	count, err := s.repo.GetUnreadCount(s.ctx, "unread-room", "user-002")
	s.NoError(err)
