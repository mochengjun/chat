...
type User struct {
	...
	PasswordHash string    `gorm:"column:password_hash" json:"-"` // 改为可选，OAuth用户可能没有密码
	PhoneNumber  *string   `gorm:"column:phone_number;uniqueIndex" json:"phone_number,omitempty"`
	Email        *string   `gorm:"column:email;index" json:"email,omitempty"` // 添加索引以便OAuth查询
	...
	// OAuth 相关字段
	AuthType      string `gorm:"column:auth_type;size:20;default:password" json:"auth_type"` // password, oauth
	EmailVerified bool   `gorm:"column:email_verified;default:false" json:"email_verified"`   // 邮箱是否已验证
...
func (r *userRepository) UpdateEmailVerified(ctx context.Context, userID string, verified bool) error {
	return r.db.WithContext(ctx).Model(&User{}).Where("user_id = ?", userID).Update("email_verified", verified).Error
}
