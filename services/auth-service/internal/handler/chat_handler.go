...
func (h *ChatHandler) ListPublicRooms(c *gin.Context) {
	userID := c.GetString("user_id")
	query := c.Query("q")

	rooms, err := h.chatService.GetPublicRooms(c.Request.Context(), userID, query)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// ====== Internal Handlers (供 cleanup-service 调用) ======

// AutoDeleteMessagesRequest 自动删除消息请求
type AutoDeleteMessagesRequest struct {
	Messages []AutoDeleteMessageItem `json:"messages" binding:"required"`
}

// AutoDeleteMessageItem 待删除的消息项
type AutoDeleteMessageItem struct {
	MessageID string `json:"message_id" binding:"required"`
	RoomID    string `json:"room_id" binding:"required"`
}

// AutoDeleteMessages 批量自动删除消息（内部接口，供 cleanup-service 调用）
func (h *ChatHandler) AutoDeleteMessages(c *gin.Context) {
	var req AutoDeleteMessagesRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid request body"})
		return
	}
	...
	for _, item := range req.Messages {
		// 执行软删除
		if err := h.chatService.DeleteMessageInternal(c.Request.Context(), item.MessageID); err != nil {
			continue
		}
		...
		// 通过 WebSocket 广播删除事件
		if h.wsHub != nil {
			h.wsHub.BroadcastToRoom(item.RoomID, WSMessage{
				Type: "message_deleted",
				Payload: gin.H{
					"room_id":    item.RoomID,
